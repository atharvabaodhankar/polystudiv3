  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f8f6ff] px-4">
      <form onSubmit={handleSignup} className="bg-white border border-[#9102C0] rounded-2xl shadow-xl p-10 flex flex-col gap-6 w-full max-w-md">
        ...
        <button
          ...
          onClick={async () => {
            if (!fullName || !branch || !year) {
              setError('Please fill in your Name, Branch, and Year before signing up with Google.');
              return;
            }
            setLoading(true);
            setError('');
        ...
        <p className="text-xs text-gray-400 text-center font-poppins mt-1">
          *Fill in Name, Branch, and Year above before using Google sign-up.
        </p>
